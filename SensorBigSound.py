from machine import Pin, ADC
import time
import network
from umqtt.simple import MQTTClient

# Configuración del sensor de sonido
sensor_pin = ADC(Pin(35))  # Pin analógico donde conectaste el sensor de sonido (ajusta según tu configuración)
sensor_pin.atten(ADC.ATTN_0DB)  # Configuración para un rango de 0 a 1.1V (ajustable según el sensor)
led_pin = Pin(16, Pin.OUT)  # Pin donde conectaste el LED

# Configuración WiFi (tu red doméstica)
WIFI_SSID = "iPhone de Noe"
WIFI_PASSWORD = "123412345"

# Configuración MQTT
MQTT_CLIENT_ID = "esp32_sensor_sonido"
MQTT_BROKER = "172.20.10.2"  # Cambia a la IP de tu broker MQTT
MQTT_PORT = 1883
MQTT_TOPIC_PUB = "ncm/sensor"  # Topic para el sensor de sonido
MQTT_TOPIC_SUB = "led/control"

# Variables para almacenar el último valor del sensor de sonido
ultimo_valor = None
umbral_cambio = 50  # Umbral para considerar un cambio significativo (ajústalo según tus necesidades)

# Función para conectar a WiFi
def conectar_wifi():
    print("Conectando a WiFi...", end="")
    sta_if = network.WLAN(network.STA_IF)
    sta_if.active(True)
    sta_if.connect(WIFI_SSID, WIFI_PASSWORD)
    while not sta_if.isconnected():
        print(".", end="")
        time.sleep(0.3)
    print("\nWiFi Conectada!")

# Función para suscribirse al broker MQTT
def subscribir():
    client = MQTTClient(MQTT_CLIENT_ID, MQTT_BROKER, port=MQTT_PORT)
    client.set_callback(llegada_mensaje)
    client.connect()
    client.subscribe(MQTT_TOPIC_SUB)
    print(f"Conectado a {MQTT_BROKER}, suscrito a {MQTT_TOPIC_SUB}")
    return client

# Función para recibir mensajes MQTT y controlar el LED
def llegada_mensaje(topic, msg):
    print(f"Mensaje recibido en {topic}: {msg}")
    if msg == b"true":
        led_pin.value(1)  # Encender LED
    elif msg == b"false":
        led_pin.value(0)  # Apagar LED

# Conectar a WiFi
conectar_wifi()

# Conectar al broker MQTT
client = subscribir()

# Ciclo infinito para leer el sensor de sonido y enviar datos solo cuando haya un cambio significativo
while True:
    client.check_msg()  # Revisar si hay mensajes en el topic de control
    try:
        # Leer el valor del sensor de sonido (0-1023 si es un ADC de 10 bits)
        valor_sonido = sensor_pin.read()

        # Calcular la diferencia con el valor anterior
        if ultimo_valor is None:
            ultimo_valor = valor_sonido  # Inicializar el valor anterior en la primera lectura

        # Si el cambio en el nivel de sonido es mayor que el umbral, publicamos el nuevo valor
        if abs(valor_sonido - ultimo_valor) > umbral_cambio:
            mensaje = f"Nivel de sonido: {valor_sonido}  db"
            print(f"Publicando: {mensaje}")
            client.publish(MQTT_TOPIC_PUB, mensaje)  # Publicamos el valor del sonido en el topic
            ultimo_valor = valor_sonido  # Actualizamos el último valor

    except OSError as e:
        print("Error al leer el sensor:", e)

    time.sleep(1)  # Esperamos 1 segundo antes de leer nuevamente el sensor
